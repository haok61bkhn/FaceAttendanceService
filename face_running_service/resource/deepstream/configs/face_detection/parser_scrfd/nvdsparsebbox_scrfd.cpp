...
int nbBindings = 10;
int mask_instance_width = 3;
int mask_instance_height = 5;  // 5 : landmark 2 top_left bottom_right
...
static bool cmp(Box a, Box b) {
  if (a.confident > b.confident) return true;
  return false;
}
...
extern "C" bool NvDsInferCustomSCRFD(
...
bool NvDsInferCustomSCRFD(
  ...
  for (int i = 1; i < nbBindings; i++) {
    std::vector<float> v{
        (float*)(outputLayersInfo[i - 1].buffer),
        (float*)(outputLayersInfo[i - 1].buffer) + outSize[i - 1]};
    outputs.push_back(v);
  }

  for (int i = 0; i < listStride.size(); i++) {
    std::vector<Box> faceObjects;
    GenerateProposals(allAnchorCenters[i], listStride[i], outputs[i * 3],
                      outputs[i * 3 + 1], outputs[i * 3 + 2], faceObjects,
                      dConfThreshold);
    predictedBoxes.insert(predictedBoxes.end(), faceObjects.begin(),
                          faceObjects.end());
  }
  ...
  for (int j = 0; j < predictedBoxes.size(); j++) {
    ...
    for (int k = 0; k < 5; k++) {
      landmark_box[3 * k + 0] = predictedBoxes[j].landmarks[k].x;
      landmark_box[3 * k + 1] = predictedBoxes[j].landmarks[k].y;
      landmark_box[3 * k + 2] = 1;
    }

    float* lm = landmark_box;
    memcpy(object.mask, lm, sizeof(float) * 15);
