...
GstElement* Tracker::Create() {
  ...
  std::string ll_config_file = tracking_config["ll_config_file"];
  if (!ll_config_file.empty()) {
    nvds_tracker_config->ll_config_file =
        strdup(((std::string)tracking_config["ll_config_file"]).c_str());
  }
  nvds_tracker_config->ll_lib_file =
      strdup(((std::string)tracking_config["ll_lib_file"]).c_str());
