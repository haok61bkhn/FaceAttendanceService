...
void FaceExtraction::Preprocess(cv::Mat& img, std::vector<float>& result) {
  int input_height_ = InputHeight();
  int input_width_ = InputWidth();
  int input_chanel_ = InputChanel();
  result.resize(input_width_ * input_height_ * input_chanel_);
  float* data = result.data();
  cv::Mat flt_img =
      cv::Mat::zeros(cv::Size(input_width_, input_height_), CV_8UC3);
  img.copyTo(flt_img(cv::Rect(0, 0, img.cols, img.rows)));
  flt_img.convertTo(flt_img, CV_32FC3, 1.f / 255);
  int channelLength = input_width_ * input_height_;
...
std::vector<std::vector<float>> FaceExtraction::ExtractFaces(
...
void FaceExtraction::ExtractFace(cv::Mat& img, std::vector<float>& feature) {
  std::vector<float> input_data_;
  Preprocess(img, input_data_);
  EngineInference(input_data_);
  feature = std::vector<float>{&output_infers_[0][0], &output_infers_[0][512]};
  L2Norm(feature);
}
}  // namespace models