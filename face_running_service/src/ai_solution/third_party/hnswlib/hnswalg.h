...
class HierarchicalNSW : public AlgorithmInterface<dist_t> {
 public:
  ...
  std::map<int, std::string> id_to_pid;
...
//                    if (candidate_id == 0) continue;
  ...
  void writeMap(std::ofstream& output, const std::map<int, std::string>& map) {
  ...
  void readMap(std::ifstream& input, std::map<int, std::string>& map) {
    size_t size;
  ...
  void loadIndex(const std::string& location, SpaceInterface<dist_t>* s,
    ...
    int file_size = static_cast<int>(total_filesize);
    if (file_size == 0) throw std::runtime_error("File empty");
    input.seekg(0, input.beg);

    readMap(input, id_to_pid);
