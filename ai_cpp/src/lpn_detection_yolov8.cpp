...
namespace models {
LPN_DETECTION_YOLOV8::LPN_DETECTION_YOLOV8(std::string& onnx_file,
...
LPN_DETECTION_YOLOV8::~LPN_DETECTION_YOLOV8() {}

bool LPN_DETECTION_YOLOV8::Preprocess(const cv::Mat& img,
...
float LPN_DETECTION_YOLOV8::Clamp(float val, float min, float max) {
  return val > min ? (val < max ? val : max) : min;
}

std::vector<LPN_DET> LPN_DETECTION_YOLOV8::Detect(const cv::Mat& img) {
...
cv::Mat LPN_DETECTION_YOLOV8::Align_LPN(const cv::Mat& img,
                                        std::vector<float>& kps) {
  cv::Mat img_crop;
  ...
  not_a_rect_shape.push_back(cv::Point(int(kps[9]), int(kps[10])));
  not_a_rect_shape.push_back(cv::Point(int(kps[3]), int(kps[4])));
...
cv::Mat LPN_DETECTION_YOLOV8::DrawLPN(cv::Mat& draw_img,
                                      std::vector<LPN_DET>& detections) {
